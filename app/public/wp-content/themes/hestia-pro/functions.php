		...
		<li class="hestia-search-in-menu">
			<form role="search" method="get" class="hestia-search-in-nav" action="' . esc_url( home_url( '/' ) ) . '">
				<div class="hestia-nav-search">
					<span class="screen-reader-text">' . _x( 'Search for:', 'label', 'hestia-pro' ) . '</span>
					<span class="search-field-wrapper">
					
					<input type="search" class="search-field" placeholder="' . esc_attr_x( 'Search &hellip;', 'placeholder', 'hestia-pro' ) . '" value="' . get_search_query() . '" name="s" />
					</span>
...
if ( function_exists( 'hestia_setup_theme' ) ) {
	...
	function hestia_fagri_notice() {
		...
		if ( ! get_user_meta( $user_id, 'hestia_ignore_fagri_notice' ) ) {
			...
			printf( esc_html__( 'We just launched a new free Hestia %s, you might like it.', 'hestia-pro' ), sprintf( '<a href="%1$s">%2$s</a>', admin_url( 'theme-install.php?theme=fagri' ), esc_html__( 'child theme', 'hestia-pro' ) ) );
			echo '</p>';
			echo '</div>';
		}
	}
