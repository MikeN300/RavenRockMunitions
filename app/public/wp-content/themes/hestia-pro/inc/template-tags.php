...
if ( ! function_exists( 'hestia_category' ) ) :
	...
	function hestia_category() {
		...
		if ( $category ) {
			...
			echo '<a href="' . esc_url( get_category_link( $category[0]->term_id ) ) . '" title="' . esc_attr( sprintf( __( 'View all posts in %s', 'hestia-pro' ), $category[0]->name ) ) . '" ' . '>' . esc_html( $category[0]->name ) . '</a> ';
		}
	}
endif;
