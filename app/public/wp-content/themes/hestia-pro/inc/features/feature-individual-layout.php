...
function hestia_individual_layout_metabox() {
	...
	if ( ! empty( $post ) ) {
		...
		if ( ( in_array( $page_template, $allowed_templates ) || empty( $page_template ) ) && ( empty( $bp_pages ) || in_array( $post->ID, $bp_pages ) === false ) ) {
			add_meta_box(
				'hestia-individual-layout', esc_html__( 'Layout', 'hestia-pro' ), 'hestia_individual_layout_metabox_content', array(
					'post',
					'page',
				), 'side', 'low'
			);
...
function hestia_individual_layout_metabox_content() {
	...
	<p>
		<select name="hestia_layout_select" id="hestia_layout_select">
			<option value="default" <?php selected( $selected, 'default' ); ?>><?php echo esc_html__( 'Default', 'hestia-pro' ); ?></option>
			<option value="full-width" <?php selected( $selected, 'full-width' ); ?>><?php echo esc_html__( 'Full Width', 'hestia-pro' ); ?></option>
			<option value="sidebar-left" <?php selected( $selected, 'sidebar-left' ); ?>><?php echo esc_html__( 'Left Sidebar', 'hestia-pro' ); ?></option>
			<option value="sidebar-right" <?php selected( $selected, 'sidebar-right' ); ?>><?php echo esc_html__( 'Right Sidebar', 'hestia-pro' ); ?></option>
		</select>
	</p>
	<?php
}
