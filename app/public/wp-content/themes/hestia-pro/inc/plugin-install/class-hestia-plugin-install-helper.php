		...
		if ( ! empty( $slug ) ) {
			...
			switch ( $state ) {
				...
				case 'activate':
					...
					$button .= '<a data-slug="' . $slug . '" class="activate-now button button-primary" href="' . esc_url( $nonce ) . '" aria-label="Activate ' . $slug . '">' . esc_html__( 'Activate', 'hestia-pro' ) . '</a>';
					break;

				case 'enable_cpt':
					$url     = admin_url( 'admin.php?page=jetpack#/settings' );
					$button .= '<a  class="button" href="' . esc_url( $url ) . '">' . esc_html__( 'Activate', 'hestia-pro' ) . ' ' . esc_html__( 'Jetpack Portfolio', 'hestia-pro' ) . '</a>';
					break;
			}// End switch().
			$button .= '</div>';
		}// End if().
