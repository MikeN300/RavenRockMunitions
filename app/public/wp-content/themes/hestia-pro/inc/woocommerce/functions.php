...
function hestia_woocommerce_header_add_to_cart_fragment( $fragments ) {
	...
	<a class="cart-contents btn btn-white pull-right" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'hestia-pro' ); ?>">
		...
		printf( _n( '%d item', '%d items', absint( $woocommerce->cart->cart_contents_count ), 'hestia-pro' ), absint( $woocommerce->cart->cart_contents_count ) );
		echo ' - ';
		echo wp_kses(
...
if ( ! function_exists( 'hestia_cart_link_after_primary_navigation' ) ) {
	...
	function hestia_cart_link_after_primary_navigation() {
		?>
		<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View cart', 'hestia-pro' ); ?>" class="nav-cart-icon">
