...
if( !class_exists( 'YITH_WCCL_Admin' ) ) {
    ...
    class YITH_WCCL_Admin {
        ...
        public function add_attribute_field( $taxonomy ) {
            ...
            do_action('yith_wccl_print_attribute_field', $attribute->attribute_type, false );
        }
        ...
        public function print_attribute_type($attribute, $value = '', $table = 0){
            ...
            if( $table ): ?>
             <tr class="form-field">
                <th scope="row" valign="top"><label for="term-value"><?php echo $custom_types['label']; ?></label></th>
                <td>
            <?php else: ?>
            <div class="form-field">
                <label for="term-value"><?php $custom_types['label']; ?></label>
            <?php endif ?>
        ...
        function product_option_terms( $tax, $i ) {
            ...
            if( in_array( $tax->attribute_type, array( 'colorpicker', 'image', 'label' ) ) ) {
                ...
                <button class="button fr plus add_new_attribute" data-attribute="<?php echo $attribute_taxonomy_name; ?>"><?php _e( 'Add new', 'yith-woocommerce-color-label-variations' ); ?></button>
                <?php
            }
        }
        ...
        public function register_panel() {
            ...
            $admin_tabs = array(
                'premium' => __( 'Premium Version', 'yith-woocommerce-color-label-variations' ),
            );

            $args = array(
                'create_menu_page' => true,
                'parent_slug'      => '',
                'page_title'       => _x( 'Color and Label Variations', 'plugin name in admin page title', 'yith-woocommerce-color-label-variations' ),
                'menu_title'       => _x( 'Color and Label Variations', 'plugin name in admin WP menu', 'yith-woocommerce-color-label-variations' ),
